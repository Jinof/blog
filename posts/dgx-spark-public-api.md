...
MiniMax H3 在 DGX Spark 上跑起来后，我想在局域网外也能调用它。我用 Docker 跑服务，在 H3 前面加 New API 管理调用令牌，再用 Tailscale Funnel 提供公网入口。这样就能从外面提交任务，把生成的视频下载回来。

这篇记录的是 2026 年 9 月的部署过程。主机名、域名和令牌已换成示例值。
...
New API 负责检查调用令牌。Funnel 只负责把请求送进来，不检查模型的 API Token。直接发布没有鉴权的 H3 端口，就等于允许知道地址的人使用这台机器生成视频。
...
`docker ps` 显示运行时，模型可能还在读权重。我等 `/health` 返回 200 后，又提交了一次生成请求，确认它能实际出结果。

这台机器的 H3 冷启动用了约 9 分钟。启动前我会先用 `free -h` 看可用内存，避免同时加载另一个大模型。
...
用 Linux host 网络，是为了让容器里的 New API 直接访问宿主机的 `127.0.0.1:8092`。普通 bridge 网络下，容器里的 `127.0.0.1` 指向容器自己。
...
我在这个版本自带的 Sora 任务插件上改了几处，让它能对接 H3：
...
改完后，4 步、2 秒的小样成功下载，MP4 为 435,871 字节。这次才完整走完提交、查询和下载。这个小样只用来检查接口，没评估画质和并发性能。
...
我又从公网访问了一遍：网页返回 200，不带令牌请求模型 API 返回 401。除了网页能打开，还要确认未授权请求确实会被拒绝。
...
任务 ID 要留好。请求超时后先查原任务的状态，直接重提可能让 Spark 把同一个视频生成两遍。
...
现在我已经能从外面调用 H3，也能给少量客户端分发令牌。并发压测还没做，数据库备份也还要安排；如果使用人数增加，再补排队、配额和监控。
